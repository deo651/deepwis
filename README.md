...
## ☁️ 在腾讯云 / 任意 Linux 服务器上运行（公网可访问）

> 本仓库已经在腾讯云 Linux 实例上验证。下列步骤适用于任何 Linux + Node 22 的环境。
...
未开放时表现为：浏览器超时 / 连接拒绝 / 空白。这一步**只能在腾讯云控制台完成**，本机无法替代。
...
- 在腾讯云安全组规则中**优先**限制来源为你自己的公网 IP/32，不要默认使用 `0.0.0.0/0`

## 📦 部署建议

- **腾讯云 / 阿里云 / AWS EC2 + Nginx**：`npm run build` → 把 `dist/` 交给 Nginx 静态托管，**LLM 模式需要保留 `vite preview` 或自建 Node 服务承担 `/api/llm/*` 代理**；如果只暴露 dist，前端会自动走 DemoProvider。
