...
| **在线访问链接** | `http://<SERVER_PUBLIC_IP>:5173`<br>（服务部署在云服务器 Linux 实例上，监听 `0.0.0.0:5173`；**正式提交时请把 `<SERVER_PUBLIC_IP>` 替换为云控制台展示的真实公网 IP，让评审无需登录直接点开**） |
| **公网访问真实状态** | **已就绪可对外访问**：服务监听 `0.0.0.0:5173`，云厂商安全组入站 TCP 5173 已放行，本机 / 内网 / 服务器侧公网 IP curl 均返回 HTTP 200，HTML 含深色主题标记 `class="dark"`。 |
| **GitHub 源码链接** | https://github.com/deo651/deepwis （main 分支） |
| **本地启动** | `npm install && npm run dev`，打开 `http://localhost:5173` |
| **云服务器启动** | `npm run build && nohup npm run preview -- --host 0.0.0.0 --port 5173 --strictPort > /tmp/deepwis-app.log 2>&1 &` |
...
> 公网 IP 与端口、所需安全组规则、Conda / Node 环境、`.env` 用法等运行细节，详见仓库根目录 `README.md` 的"☁️ 在云服务器 / 任意 Linux 主机上运行"章节。

### 30 秒上手（评审快速路径）

1. 打开访问链接 → 默认进入深色 Agent Workspace
2. 左侧 Sidebar → 点任意一个**模板**（推荐"极简任务清单"）→ 立刻看到 Agent 5 步轨迹与可交互预览
3. 在预览里**新建一条任务** → 勾选完成 → 看统计数字变化 → **F5 刷新** → 数据仍在（IndexedDB）
4. 中间输入框 → 发"加近 7 天完成趋势图" → 看 Agent 5 步执行 → 生成 v2 含新增趋势图
5. 切到 **Diff Tab** → 看 v1 → v2 的红绿 schema 差异 → **一键恢复**到 v1 → 自动生成 v3 可追溯
...
> 执行环境：云服务器 Linux + Conda 环境 `deepwis`（Python 3.10.0）+ Node v22.22.0 + npm 10.9.4。所有命令均在仓库根目录 `/root/deepwis` 下运行。
...
npm run build        ✓  vite build, 2408 modules, 6.58s
                        dist/index.html             0.64 kB │ gzip 0.44 kB
                        dist/assets/index-*.css    28.29 kB │ gzip 5.58 kB
                        dist/assets/index-*.js    636.28 kB │ gzip 185.68 kB
```
...
- **应用层 / 端口绑定层 / 系统防火墙层 / 云厂商安全组层** 全部放行完成
- 公网访问入口：`http://<SERVER_PUBLIC_IP>:5173`（替换占位符为真实公网 IP 即可）
- 长期对外分享建议进一步加上 TLS + 域名（见 P0 扩展计划）
